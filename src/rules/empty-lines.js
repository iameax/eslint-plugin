/**
 * @fileoverview Manage empty lines.
 * implementation adapted from the no-multiple-empty-lines rule.
 */
'use strict';
    ...
    return {
      ...
      ImportDeclaration(node) {
        ...
        if (nextNode && nextNode.type !== 'ImportDeclaration' && (nextNode.type !== 'TSImportEqualsDeclaration' || nextNode.isExport)) {
          const importDeclarationText = sourceCode.getText(node);
          endOfImportsLineNum = allLines.findIndex(line => line === importDeclarationText) + 1;
        }
      },
      'Program:exit'(node) {
        ...
        lineNums.reduce((lastLineNumber, lineNumber, _, arr) => {
          ...
          if (actual < min) {
            context.report({
              node,
              loc: {
                start: { line: lastLineNumber, column: 0 },
              },
              messageId: 'def',
              data: {
                rule: `${rule}_min=${min}`,
              ...
              fix(fixer) {
                const range = lineNumber <= allLines.length
